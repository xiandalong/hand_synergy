...
for j=1:num_trials
    ...
    joint_angles_LeftHand = getJointAngles(marker_pos,...
    ...
    if regexp(Grasping_Phase,'holding')
        if regexp(mimicHand,'left')
            joint_angles_LeftHand = joint_angles_LeftHand(:,Data_table.leftMimic_start_frame(j):Data_table.leftMimic_start_frame(j)+frame_rate*duration-1);
            joint_angles_RightHand = joint_angles_RightHand(:,Data_table.leftMimic_start_frame(j):Data_table.leftMimic_start_frame(j)+frame_rate*duration-1);
        elseif regexp(mimicHand,'right')
            joint_angles_LeftHand = joint_angles_LeftHand(:,Data_table.rightMimic_start_frame(j):Data_table.rightMimic_start_frame(j)+frame_rate*duration-1);
            joint_angles_RightHand = joint_angles_RightHand(:,Data_table.rightMimic_start_frame(j):Data_table.rightMimic_start_frame(j)+frame_rate*duration-1);
        end
    elseif regexp(Grasping_Phase,'reaching')
        if regexp(mimicHand,'left')
            joint_angles_LeftHand = joint_angles_LeftHand(:,Data_table.LM_leftReaching_start(j):Data_table.LM_lefReaching_end(j));
            joint_angles_RightHand = joint_angles_RightHand(:,Data_table.LM_rightReaching_start(j):Data_table.LM_rightReaching_end(j));
        elseif regexp(mimicHand,'right')
            joint_angles_LeftHand = joint_angles_LeftHand(:,Data_table.RM_leftReaching_start(j):Data_table.RM_lefReaching_end(j));
            joint_angles_RightHand = joint_angles_RightHand(:,Data_table.RM_rightReaching_start(j):Data_table.RM_rightReaching_end(j));
        end
    end
...
%     %
    ...
    Data_table.MGA_LeftHand{j} = GA_LeftHand;
    Data_table.MGA_RightHand{j} = GA_RightHand;
end
...
for j=1:num_trials
    raw_data_Left(j,:)= ( mean(Data_table.joint_angles_LeftHand{j},2))';
end
...
for j=1:num_trials
    raw_data_Right(j,:)= ( mean(Data_table.joint_angles_RightHand{j},2))';
end
