...
% This version uses marker position rather than the joint angles as the
% input for PCA
...
for j=1:size(table,1)
    ...
    marker_pos_LeftHand = marker_pos_LeftHand(:,table.left_start_frame(j):table.left_start_frame(j)+frame_rate*duration-1);
    marker_pos_RightHand = marker_pos_RightHand(:,table.left_start_frame(j):table.left_start_frame(j)+frame_rate*duration-1);
    
    % put the joint angles into "Data_array"
    Data_array{j,6} = marker_pos_LeftHand;
    Data_array{j,7} = marker_pos_RightHand;
...
raw_data_Left = zeros(num_trials,34*3);
...
% Right Hand
num_trials = size(Data_array,1);
raw_data_Right = zeros(num_trials,34*3);
for j=1:size(Data_array,1)
